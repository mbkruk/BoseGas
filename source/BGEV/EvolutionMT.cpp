...
void BGEvolution::derivative1(const __m256d *r, const int_fast32_t i)
{
	barrier.wait();
...
void BGEvolution::thread(const int_fast32_t i)
{
	...
	while (batches--)
	{
		...
		while (steps--)
		{
			...
			yk1[i] = pCur[i]+k1[i]*b[1];

			derivative1(yk1,i);
			k2[i] = *(pDerivative+i)*H;
			yk2[i] = *(pCur+i)+*(k1+i)*b[2]+*(k2+i)*b[3];

			derivative1(yk2,i);
			k3[i] = *(pDerivative+i)*H;
			yk3[i] = *(pCur+i)+*(k1+i)*b[4]+*(k2+i)*b[5]+*(k3+i)*b[6];

			derivative1(yk3,i);
			k4[i] = *(pDerivative+i)*H;
			yk4[i] = *(pCur+i)+*(k1+i)*b[7]+*(k2+i)*b[8]+*(k3+i)*b[9]+*(k4+i)*b[10];

			derivative1(yk4,i);
			k5[i] = *(pDerivative+i)*H;
			yk5[i] = *(pCur+i)+*(k1+i)*b[11]+*(k2+i)*b[12]+*(k3+i)*b[13]+*(k4+i)*b[14]+*(k5+i)*b[15];

			derivative1(yk5,i);
			k6[i] = *(pDerivative+i)*H;

			pCur[stride+i] = *(pCur+i)+*(k1+i)*c[1]+*(k3+i)*c[3]+*(k4+i)*c[4]+*(k6+i)*c[6];
...
void BGEvolution::evolve2()
{
	...
	for (int_fast32_t j=0;j<stride;++j)
		*(initial_conditions+j) = *(pCurrent+j);
}
